...
class Command(BaseCommand):
    FILES_AND_MODELS_NAME = {
        'ingredients.csv': {'model_obj': Ingredient},
    }
    ...
    def load_data_from_csv():
        for file_name, data in Command.FILES_AND_MODELS_NAME.items():
            ...
            objs = []
            for row in reader:
                objs.append(model_object(**row))
            model_object.objects.bulk_create((objs), ignore_conflicts=True)
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            raise CommandError('Объект не может быть создан', error)
